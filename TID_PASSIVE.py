...
SIM_PROTONS = 100000000
SIM_ELECTRONS = 20000000
...
O1_E_PASS_FILES = [f"{path}E1_T_00_Al_00.csv", f"{path}E1_T_00_Al_01.csv", f"{path}E1_T_00_Al_02.csv",
                   f"{path}E1_T_00_Al_03.csv", f"{path}E1_T_00_Al_05.csv", f"{path}E1_T_00_Al_10.csv"]
...
tid_o1_p_pass = [calculate_tid(f, SIM_PROTONS, FLUX_O1_P, MISSION_DAYS, TARGET_MASS_KG) for f in O1_P_PASS_FILES]
tid_o1_e_pass = [calculate_tid(f, SIM_ELECTRONS, FLUX_O1_E, MISSION_DAYS, TARGET_MASS_KG) for f in O1_E_PASS_FILES]
tid_o1_tot_pass = [p + e for p, e in zip(tid_o1_p_pass, tid_o1_e_pass)]
# Active
# tid_o1_p_act  = [calculate_tid(f, SIM_PROTONS, FLUX_O1_P, MISSION_DAYS, TARGET_MASS_KG) for f in O1_P_ACT_FILES]
# tid_o1_e_act  = [calculate_tid(f, SIM_ELECTRONS, FLUX_O1_E, MISSION_DAYS, TARGET_MASS_KG) for f in O1_E_ACT_FILES]
# tid_o1_tot_act  = [p + e for p, e in zip(tid_o1_p_act, tid_o1_e_act)]

# CALCULATE DOSES - ORBIT 2
tid_o2_p_pass = [calculate_tid(f, SIM_PROTONS, FLUX_O2_P, MISSION_DAYS, TARGET_MASS_KG) for f in O2_P_PASS_FILES]
tid_o2_e_pass = [calculate_tid(f, SIM_ELECTRONS, FLUX_O2_E, MISSION_DAYS, TARGET_MASS_KG) for f in O2_E_PASS_FILES]
tid_o2_tot_pass = [p + e for p, e in zip(tid_o2_p_pass, tid_o2_e_pass)]

# tid_o2_p_act  = [calculate_tid(f, SIM_PROTONS, FLUX_O2_P, MISSION_DAYS, TARGET_MASS_KG) for f in O2_P_ACT_FILES]
# tid_o2_e_act  = [calculate_tid(f, SIM_ELECTRONS, FLUX_O2_E, MISSION_DAYS, TARGET_MASS_KG) for f in O2_E_ACT_FILES]
